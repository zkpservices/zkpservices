...
## Remove a service from a users dashboard {{ tag: 'DELETE', label: '/userdata, action: "remove_from_dashbaord"' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl --location --request DELETE 'https://y1oeimdo63.execute-api.us-east-1.amazonaws.com/userdata' \
    --header 'Content-Type: application/json' \
    --data-raw '{
        "id": "Ox8ab74ba8cbed8374bd",
    ...
    $response = Invoke-RestMethod 'https://y1oeimdo63.execute-api.us-east-1.amazonaws.com/userdata' -Method 'DELETE' -Headers $headers -Body $body
    $response | ConvertTo-Json
    ```
