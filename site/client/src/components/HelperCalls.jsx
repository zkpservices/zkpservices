...
export async function generateCoreProof(inputs) {
  ...
  const input = {
    ...
    provided_field_and_salt_and_user_secret_hash:
      inputs.provided_field_and_salt_and_user_secret_hash,
    provided_salt_hash: inputs.provided_salt_hash,
  }

  const circuitWasm =
    '/circuit_wasms_and_keys/ZKPServicesCoreResponse/build/ZKPServicesCoreResponse_js/ZKPServicesCoreResponse.wasm'
  const circuitZkey =
    '/circuit_wasms_and_keys/ZKPServicesCoreResponse/circuit.zkey'

  let vKey
  const response = await fetch(
    '/circuit_wasms_and_keys/ZKPServicesCoreResponse/verification_key.json',
  )
  if (response.ok) {
...
export async function generate2FAProof(inputs) {
  ...
  const circuitWasm =
    '/circuit_wasms_and_keys/ZKPServicesVRF2FAResponse/build/ZKPServicesVRF2FAResponse_js/ZKPServicesVRF2FAResponse.wasm'
  const circuitZkey =
    '/circuit_wasms_and_keys/ZKPServicesVRF2FAResponse/circuit.zkey'

  let vKey
  const response = await fetch(
    '/circuit_wasms_and_keys/ZKPServicesVRF2FAResponse/verification_key.json',
  )
  if (response.ok) {
    vKey = await response.json()
    console.log('vKey:', vKey)
...
export async function generatePasswordChangeProof(inputs) {
  ...
  const circuitWasm =
    '/circuit_wasms_and_keys/ZKPServicesVRF2FAPasswordChange/build/ZKPServicesVRF2FAPasswordChange_js/ZKPServicesVRF2FAPasswordChange.wasm'
  const circuitZkey =
    '/circuit_wasms_and_keys/ZKPServicesVRF2FAPasswordChange/circuit.zkey'

  let vKey
  const response = await fetch(
    '/circuit_wasms_and_keys/ZKPServicesVRF2FAPasswordChange/verification_key.json',
  )
  if (response.ok) {
    vKey = await response.json()
    console.log('vKey:', vKey)
