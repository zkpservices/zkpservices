...
export function ThreeJSComponent() {
  ...
  useEffect(() => {
    ...
    if (!showLogo) {
      ...
      if (window.innerWidth >= 1650) {
        ...
        canvasHeight =
          (containerRef.current.clientWidth * 1.3) /
          (window.innerWidth / window.innerHeight)
      } else if (window.innerWidth >= 1200) {
        canvasWidth = containerRef.current.clientWidth * 1.15
        canvasHeight =
          (containerRef.current.clientWidth * 1.15) /
          (window.innerWidth / window.innerHeight)
      } else if (window.innerWidth >= 768) {
        canvasWidth = containerRef.current.clientWidth * 0.9
        canvasHeight =
          (containerRef.current.clientWidth * 0.9) /
          (window.innerWidth / window.innerHeight)
      }
        ...
        .then((text) => {
          ...
          const animate = function () {
            ...
            if (isMouseMoving && window.innerWidth > 450) {
              ...
              rotationY = Math.max(
                -maxRotationY,
                Math.min(maxRotationY, rotationY),
              )
  ...
  return (
    <div className="flex w-full items-center justify-center" ref={containerRef}>
      {showLogo ? (
        <div>
          <Logo className="mb-20 mt-10 h-48" />
        </div>
      ) : null}
    </div>
  )
